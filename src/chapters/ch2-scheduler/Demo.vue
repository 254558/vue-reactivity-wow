  ...
  <div class="demo-layout">
    <div class="main-grid">
      ...
      <div class="right-col">
        ...
        <div class="viz-card">
          <div class="viz-head"><i class="fa-solid fa-code-compare"></i>
            <h4>累计执行次数对比</h4>
          </div>
        ...
        <div class="viz-card action">
          <button class="inc-btn" :class="{ pulse: store.canIncrement }" @click="store.simulateIncrement()"
            :disabled="!store.canIncrement || store.isFlushing">
    ...
    <div class="controls">
      <button class="ctrl-btn outline" @click="store.prevStep()" :disabled="store.currentStep <= 1"><i
          class="fa-solid fa-chevron-left"></i> 上一步</button>
      <div class="step-tabs">
...
.g-branch.yes .branch-label {
  color: var(--accent);
}

.g-branch.no .branch-label {
  color: var(--crimson);
}
...
.queue-slot {
  flex: 1;
  height: 24px;
  border: 1px dashed rgba(94, 138, 118, 0.2);
  border-radius: 4px;
  display: flex;
  align-items: center;
  justify-content: center;
...
.compare-item.bad .compare-val {
  color: var(--crimson);
}

.compare-item.good .compare-val {
  color: var(--accent);
}
...
@keyframes node-flash {
  ...
  50% {
    box-shadow: 0 0 24px 6px rgba(16, 185, 129, 0.2)
  }
