...
export const steps = [
  ...
  },
  {
    ...
    label: '追踪依赖',
    desc: '调用 effect(getter, { scheduler })。effect 会立即执行一次 getter 来追踪其内部读取的响应式数据。当数据后续变化时，不再重新执行 getter，而是执行 scheduler 仅标记为脏。',
    detail: 'const effectFn = effect(getter, {\n  scheduler() { ... }\n})\n\n创建时：立即执行 getter 一次 (追踪依赖)\n数据变化时：不执行 effectFn (不重新计算)\n而是执行 scheduler (标记为脏)',
    color: '#10b981',
